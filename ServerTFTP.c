...
void ErrorMessage(char ep[4], short int errMessage){
...
void enDP(char dp[516], short int no, char data[512]){
...
void deACK(char code[4], short int *opcode, short int *blockno){
	short int tmp;
...
int main(){
	...
	if(Op(rd)==1 || Op(rd)==2){
		...
		//error
		char errMessage[128];
		DecodeErrorMessage(r, rd, errMessage);
	...
	if(opcode == 1){
		...
		while((byteread = read(fd, buf, sizeof(buf)))!=0){
			...
			if(Op(ack)==5){
				//error
				char errMessage[128];
                		DecodeErrorMessage(r, ack, errMessage);
 		                printf("!!! got err message : %s!!!\n", errMessage);
				return 1;
			}else if(Op(ack)==4){
				deACK(ack, &opcode2, &blockno2);
