...
void enErrorMessage(char ep[4], short int errMessage){
...
void decodeErrorMessage(int r, char code[], char errMessage[128]){
        short int tmp, en ;
        memcpy(&tmp, code+2, 2);
        en = ntohs(tmp);
        switch(en){
...
void deDP(char dp[516], int r, short int *opcode, short int *blockno, char data[512]){
	short int tmp;
	memcpy(&tmp, dp, 2);
	*opcode = ntohs(tmp);
	memcpy(&tmp, dp+2, 2);
...
void enACK(char ack[4], short blockno){	
	short int tmp;
	tmp = htons(4);
	memcpy(ack, &tmp, 2);
	tmp = htons(blockno);
...
int enRW(char bd[139], short int opcode, char filename[128], char mode[9]){
...
int main(){
	...
	char bd[139];
        int len = enRW(bd, 1, "ReadFile.txt", "octet");
	...
	if(checkOp(dp)==5){
		//error
		char errMessage[128];
                decodeErrorMessage(r, dp, errMessage);
                printf("!!! got err message : %s!!!\n", errMessage);
		return 1;
	}else if(checkOp(dp)==3){
		deDP(dp, r, &opcode, &blockno, data);
		printf("got dp block = %d\n", blockno);
		printf("========data==============\n%s\n===============\n", data);
		int fd = open("WritFile.txt", O_WRONLY);
		...
		while(1){
			...
			if(checkOp(dp)==5){
				...
				deDP(dp, r, &opcode2, &blockno3, data);
				printf("got dp block = %d\n", blockno3);
				printf("========data==============\n%s\n===============\n", data);
				blockno2++;
			}
